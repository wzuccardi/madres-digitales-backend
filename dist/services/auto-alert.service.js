...
exports.AutoAlertService = void 0;
const logger_1 = require("../config/logger");
class AutoAlertService {
    constructor(prisma, alertRulesEngine) {
        this.prisma = prisma;
     ...
     */
    async evaluateAndCreateAlert(controlData, sintomas = []) {
        try {
            ...
            for (const evaluacion of evaluacionSignos) {
                if (evaluacion.alertDetected) {
                    ...
                    logger_1.log.info('Alerta automática creada', {
            ...
            logger_1.log.error('Error en evaluación automática de alertas', { error: error.message, controlData });
            throw error;
        }
    }
    /**
     ...
     */
    async evaluateRiskPatterns(gestanteId) {
        try {
            ...
            logger_1.log.error('Error evaluando patrones de riesgo', { error: error.message, gestanteId });
            return [];
        }
    }
    /**
     ...
     */
    async processVitalSigns(gestanteId, vitalSigns, sintomas = []) {
        try {
     ...
     */
    async getAutoAlertStats(fechaInicio, fechaFin) {
        try {
    ...
    evaluarTendenciaPresion(controles) {
        const controlesConPresion = controles.filter(c => c.presion_sistolica && c.presion_diastolica);
...
exports.AutoAlertService = AutoAlertService;

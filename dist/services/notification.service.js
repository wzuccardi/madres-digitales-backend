...
class NotificationService {
    ...
    async procesarAlertaParaNotificaciones(alertaId) {
        ...
        try {
            ...
            if (!this.debeNotificar(alerta.nivel_prioridad)) {
     ...
     */
    async determinarDestinatarios(alerta, gestante) {
        const destinatarios = [];
        // Madrina asignada
        ...
        if (this.configuracion.notificar_coordinador && gestante.municipio_id) {
            const coordinadores = await database_1.default.usuario.findMany({
                where: {
                    rol: 'coordinador',
                    activo: true
     ...
     */
    async generarNotificaciones(alerta, gestante, destinatarios) {
     ...
     */
    determinarCanales(prioridad, tipoDestinatario) {
        const canales = [];
        // Para alertas críticas, usar todos los canales disponibles
        if (prioridad === 'critica') {
            if (this.configuracion.usar_sms)
                canales.push('sms');
            if (this.configuracion.usar_push)
                canales.push('push');
            if (this.configuracion.usar_email)
                canales.push('email');
            if (this.configuracion.usar_whatsapp)
     ...
     */
    generarMensaje(alerta, gestante, destinatario, canal) {
     ...
     * @returns true si exitoso
     */
    async enviarWhatsApp(notificacion) {
    ...
    formatearUbicacion(coordenadas) {
        if (!coordenadas)
            return null;
        try {
     ...
     */
    async notifyAlertUpdate(alertaId, updateType) {
        if (!this.webSocketService)
            return;
        try {
            const alerta = await database_1.default.alerta.findUnique({

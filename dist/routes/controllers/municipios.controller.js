    ...
    try {
        ...
        const municipiosConEstadisticas = municipios.map(municipio => {
            ...
            return {
                ...
                poblacion: null,
                latitud: municipio.coordenadas ? municipio.coordenadas.coordinates?.[1] : null,
                longitud: municipio.coordenadas ? municipio.coordenadas.coordinates?.[0] : null,
...
const getResumenIntegrado = async (req, res) => {
    try {
        ...
        const ipsActivas = await database_1.default.ips.count({ where: { activa: true } });
        ...
        const resumen = {
            ...
            distribucion_niveles_atencion: distribucionNiveles.reduce((acc, item) => {
                acc[item.nivel_atencion] = item._count.id;
                return acc;
            }, {}),

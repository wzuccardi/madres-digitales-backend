...
async function main() {
    ...
    try {
        ...
        if (municipios.length < 3) {
            throw new Error('No hay suficientes municipios en la base de datos. Ejecuta primero el script de importación de municipios.');
        }
        console.log(`📍 Usando municipios: ${municipios.map(m => m.nombre).join(', ')}`);
        // Extraer coordenadas de los municipios (están en formato GeoJSON)
        const coordenadasMunicipios = municipios.map(m => {
            const coords = m.coordenadas;
            if (coords && coords.coordinates) {
                return {
                    longitud: coords.coordinates[0],
                    latitud: coords.coordinates[1]
                };
            }
            return { longitud: -75.5, latitud: 10.4 }; // Coordenadas por defecto para Bolívar
        });
        // Crear 4 madrinas (una coordinadora y 3 madrinas)
        const hashedPassword = await bcrypt_1.default.hash('madrina123', 10);
        // 1. Coordinadora
        const coordinadora = await prisma.usuario.create({
            data: {
                ...
                coordenadas: {
                    type: 'Point',
                    coordinates: [coordenadasMunicipios[0].longitud, coordenadasMunicipios[0].latitud]
                },
            }
        });
        // 2-4. Tres madrinas
        ...
        for (let i = 0; i < 3; i++) {
            const madrina = await prisma.usuario.create({
                data: {
                    ...
                    coordenadas: {
                        type: 'Point',
                        coordinates: [coordenadasMunicipios[i].longitud, coordenadasMunicipios[i].latitud]
                    },
                }
            });
        ...
        for (let i = 0; i < 10; i++) {
            ...
            const gestante = await prisma.gestante.create({
                data: {
                    ...
                    riesgo_alto: Math.random() > 0.7, // 30% de probabilidad de alto riesgo
                    activa: true,
                    ...
                    coordenadas: {
                        type: 'Point',
                        coordinates: [
                            coordenadasMunicipios[municipioIndex].longitud + (Math.random() - 0.5) * 0.01,
                            coordenadasMunicipios[municipioIndex].latitud + (Math.random() - 0.5) * 0.01
                        ]
                    },
                }
            });
        ...
        for (let i = 0; i < 15; i++) {
            ...
            await prisma.controlPrenatal.create({
                data: {
                    ...
                    observaciones: `Control prenatal semana ${semanasGestacion}. Evolución normal.`,
                }
            });
        }
        ...
        for (let i = 0; i < 8; i++) {
            ...
            // Obtener coordenadas de la gestante
            const coordsGestante = gestante.coordenadas;
            const coordenadasAlerta = coordsGestante && coordsGestante.coordinates ? {
                type: 'Point',
                coordinates: coordsGestante.coordinates
            } : undefined;
            const alertaData = {
                gestante_id: gestante.id,
                tipo_alerta: tiposAlerta[Math.floor(Math.random() * tiposAlerta.length)],
                nivel_prioridad: nivelesPrioridad[Math.floor(Math.random() * nivelesPrioridad.length)],
                mensaje: `Alerta generada para ${gestante.nombre}`,
                created_at: new Date(Date.now() - Math.random() * 7 * 24 * 60 * 60 * 1000), // Última semana
                resuelta: Math.random() > 0.7, // 30% resueltas
            };
            if (coordenadasAlerta) {
                alertaData.coordenadas_alerta = coordenadasAlerta;
            }

...
async function checkAndCreateAlerts() {
    try {
        ...
        const totalAlertas = await database_1.default.alerta.count();
        ...
        const gestante = await database_1.default.gestante.findFirst({
            select: {
                id: true,
                nombre: true,
                documento: true
        ...
        if (totalAlertas === 0) {
            ...
            // Alerta crítica
            const alertaCritica = await database_1.default.alerta.create({
                data: {
            ...
            // Alerta alta
            const alertaAlta = await database_1.default.alerta.create({
                data: {
                    gestante_id: gestante.id,
            ...
            // Alerta media
            const alertaMedia = await database_1.default.alerta.create({
                data: {
                    gestante_id: gestante.id,
            ...
            // Alerta baja
            const alertaBaja = await database_1.default.alerta.create({
                data: {
                    gestante_id: gestante.id,
            ...
            const alertaResuelta = await database_1.default.alerta.create({
                data: {
                    gestante_id: gestante.id,
                    tipo_alerta: 'control_vencido',
                    nivel_prioridad: 'media',
        ...
        const alertasResueltas = await database_1.default.alerta.count({ where: { resuelta: true } });
        const alertasCriticas = await database_1.default.alerta.count({
            where: { nivel_prioridad: 'critica', resuelta: false }
        });
        const alertasAltas = await database_1.default.alerta.count({
            where: { nivel_prioridad: 'alta', resuelta: false }
        });
        const alertasMedias = await database_1.default.alerta.count({
            where: { nivel_prioridad: 'media', resuelta: false }
        });
        const alertasBajas = await database_1.default.alerta.count({

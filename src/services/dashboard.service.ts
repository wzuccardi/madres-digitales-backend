...
export class DashboardService {
	async getEstadisticasGenerales() {
		try {
			...
			const [
				...
				prisma.gestante.count({ where: { activa: true } }),
				prisma.gestante.count({ where: { activa: true } }), // Temporal: usar activa hasta definir campo riesgo_alto
				prisma.controlPrenatal.count(),
				prisma.alerta.count(),
				prisma.alerta.count({ where: { resuelta: false } }),
				prisma.alerta.count({ where: { resuelta: true } }),
				prisma.alerta.count({
					where: {
						resuelta: false,
			...
			const controlesUltimoMes = await prisma.controlPrenatal.count({
				where: {
					fecha_control: {
						gte: fechaHaceUnMes
					}
			...
			const totalMedicos = await prisma.medico.count({ where: { activo: true } });
			const totalIps = await prisma.iPS.count();

			// Calcular promedio de edad gestacional real
			const gestantesConEdad = await prisma.gestante.findMany({
				where: {
	...
	async getEstadisticasPorPeriodo(fechaInicio?: string, fechaFin?: string) {
		try {
			...
			const alertasResueltasConTiempo = await prisma.alerta.findMany({
				where: {
					resuelta: true,
